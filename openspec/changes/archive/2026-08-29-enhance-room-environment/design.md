...
- **D3 — Mood state**: `mood: 'day' | 'dusk'` in the zustand scene store with localStorage persistence (try/catch, default dusk — Diego prefers the night view). A sun/moon DOM button in `Overlay` (always visible, ≥44px). Consumers read mood: `Lights` (directional/hemisphere intensity + color, accent light intensity), `FakeWindow` view, LED/arcade/logo halos unchanged (already emissive). Transition: lerp light intensities/colors over ~1s in a `useFrame` that invalidates until settled — works under demand frameloop.
