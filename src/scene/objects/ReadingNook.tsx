...
export function ReadingNook(props: GroupProps) {
  return (
    <group {...props}>
      ...
      <KenneyModel model="pillow" position={[0, 0.095, 0]} rotation={[-Math.PI / 2 + 0.06, 0.3, 0]} castShadow={false} />
      {/* open book: two halves in a shallow V, white pages on top */}
      <group position={[0.42, 0, 0.3]} rotation={[0, -0.5, 0]}>
      ...
      <mesh position={[0.5, 0.02, 0.02]} rotation={[0, 0.35, 0]} castShadow={false}>
        <boxGeometry args={[0.17, 0.04, 0.24]} />
        <meshStandardMaterial color={palette.sky} roughness={0.85} />
      </mesh>
      {/* notebook and pen */}
      <mesh position={[-0.05, 0.008, 0.48]} rotation={[0, 0.7, 0]}>
        <boxGeometry args={[0.16, 0.016, 0.22]} />
        <meshStandardMaterial color={palette.cream} roughness={1} />
      </mesh>
      <mesh position={[0.14, 0.012, 0.52]} rotation={[0, 0.25, Math.PI / 2]}>
